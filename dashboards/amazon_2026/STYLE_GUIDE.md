...
- `assets/logo/logo_light.svg` — dark grey (`#495057`) wordmark, shown in
  light mode (`#logo-light`, Vizro's naming: the logo for the *light* theme).
- `assets/logo/logo_dark.svg` — light grey (`#ced4da`) wordmark, shown in
  dark mode (`#logo-dark`). Same colors as `--na-loader-color`.
- Vizro auto-discovers `logo_light.*`/`logo_dark.*` anywhere under the assets
  folder (recursive search) and toggles visibility via `[data-bs-theme]` —
  no Python wiring needed.
...
| Control                     | Class                                                                                   | Style                                                                                                                                                                                                                                                                                          |
| --------------------------- | --------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Search input                | `.amazon-discover-search`                                                               | `dcc.Input` inside `.amazon-discover-search-input-wrap` (`position: relative`), height 38px, radius 8px, border `--border`, background `--dropdown-bg`, color `--text`, focus border `--link`, right padding reserved for the clear button; a `close` icon-button (`.amazon-discover-search-clear`, same circular style as `.amazon-discover-reference-clear`) sits absolutely positioned at the right edge and only shows once there's text (`.is-visible`); `assets/amazon_2026_discover.js` keeps `.is-visible` in sync with every keystroke (native `input` event, ahead of the debounced Dash value) and forwards a click anywhere in the wrap to focus the input |
...
| Similarity slider            | `.amazon-discover-similarity-slider`                                                    | `dcc.Slider` (0–10, step 0.1, default 5, marks `{0:"Close", 10:"Far"}`) below the reference box, themed like `.amazon-discover-rangeslider`; when a reference is selected, maps linearly to a UMAP-space radius: `radius = (value/100) × diagonal` where `diagonal` is the bounding-box diagonal of all records; at 0 only the reference itself qualifies, at 10 a tenth of the data cloud diagonal is included. Wired as `Input` to both `_update_discover_clusters` (draws a dashed circle on the UMAP) and `_update_discover_results` (filters the table to records within that radius via `filter_discover_records` `similarity_radius` kwarg). |
